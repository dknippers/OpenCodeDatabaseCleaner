...
OpenCode stores current messages in `message` and `part`, with historical copies in `event`. This utility retains and sanitizes every current `message` row older than the cutoff so message identities, parent relationships, tokens, cost, model identifiers, roles, and timestamps remain available. Every retained message has exactly one ordinary text part containing `<cleaned>`; the cleaner prefers its first non-synthetic text part, falls back to its first synthetic text part, or generates one, then deletes its other parts. Required V1 message fields remain structurally valid with sensitive values emptied. Matching durable V1 message and part events are sanitized or deleted to mirror the retained message and placeholder part. Session rows and session events are not changed.
...
Before any cleanup, the utility displays the number of selected messages, current message and part rows to change, and historical event rows to change. The current-row count covers every update and deletion in `message` and `part`. It proceeds only when `REMOVE` is entered exactly.

## Safety

- Close OpenCode before running cleanup.
- Back up the database if the history matters; cleanup cannot be undone, original part content and non-retained message events are deleted, and sensitive fields are removed from every retained message row.
...
- Databases containing non-empty V2 `session_message`, `session_input`, or `session.next.*` event storage are rejected. Partial cleanup would give a false sense that content was removed.
...
- Durable events matching an old current message are cleaned with it. Event-only history and session events are not changed. If durable events are replicated elsewhere, replaying an original copy can restore deleted content or parts; clean or disable every replica first.
